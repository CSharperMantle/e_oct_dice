...
int mpu_read_6500_accel_bias(long *accel_bias) {
	unsigned char data_[6];
	if (i2c_read(st.hw->addr, 0x77, 2, &data_[0]))
		return -1;
	if (i2c_read(st.hw->addr, 0x7A, 2, &data_[2]))
		return -1;
	if (i2c_read(st.hw->addr, 0x7D, 2, &data_[4]))
		return -1;
...
int mpu_read_6050_accel_bias(long *accel_bias) {
	unsigned char data_[6];
	if (i2c_read(st.hw->addr, 0x06, 2, &data_[0]))
		return -1;
	if (i2c_read(st.hw->addr, 0x08, 2, &data_[2]))
		return -1;
	if (i2c_read(st.hw->addr, 0x0A, 2, &data_[4]))
		return -1;
	accel_bias[0] = ((long)data_[0]<<8) | data_[1];
	accel_bias[1] = ((long)data_[2]<<8) | data_[3];
	accel_bias[2] = ((long)data_[4]<<8) | data_[5];
...
int mpu_read_6500_gyro_bias(long *gyro_bias) {
	unsigned char data_[6];
	if (i2c_read(st.hw->addr, 0x13, 2, &data_[0]))
		return -1;
	if (i2c_read(st.hw->addr, 0x15, 2, &data_[2]))
		return -1;
	if (i2c_read(st.hw->addr, 0x17, 2, &data_[4]))
		return -1;
...
int mpu_set_gyro_bias_reg(long *gyro_bias)
{
    ...
    if (i2c_write(st.hw->addr, 0x13, 2, &data_[0]))
        return -1;
    if (i2c_write(st.hw->addr, 0x15, 2, &data_[2]))
        return -1;
    if (i2c_write(st.hw->addr, 0x17, 2, &data_[4]))
        return -1;
    return 0;
}
...
int mpu_set_accel_bias_6050_reg(const long *accel_bias)
{
    ...
    if (i2c_write(st.hw->addr, 0x06, 2, &data_[0]))
        return -1;
    if (i2c_write(st.hw->addr, 0x08, 2, &data_[2]))
        return -1;
    if (i2c_write(st.hw->addr, 0x0A, 2, &data_[4]))
        return -1;
...
int mpu_set_accel_bias_6500_reg(const long *accel_bias) {
    ...
    if (i2c_write(st.hw->addr, 0x77, 2, &data_[0]))
        return -1;
    if (i2c_write(st.hw->addr, 0x7A, 2, &data_[2]))
        return -1;
    if (i2c_write(st.hw->addr, 0x7D, 2, &data_[4]))
        return -1;
...
static int compass_self_test(void)
{
    ...
    result = 0;
#if defined MPU9150
    data_ = (short)(tmp[1] << 8) | tmp[0];
    if ((data_ > 100) || (data_ < -100))
        result |= 0x01;
    data_ = (short)(tmp[3] << 8) | tmp[2];
    if ((data_ > 100) || (data_ < -100))
        result |= 0x02;
    data_ = (short)(tmp[5] << 8) | tmp[4];
    if ((data_ > -300) || (data_ < -1000))
        result |= 0x04;
#elif defined MPU9250
    data_ = (short)(tmp[1] << 8) | tmp[0];
    if ((data_ > 200) || (data_ < -200))  
        result |= 0x01;
    data_ = (short)(tmp[3] << 8) | tmp[2];
    if ((data_ > 200) || (data_ < -200))  
        result |= 0x02;
    data_ = (short)(tmp[5] << 8) | tmp[4];
    if ((data_ > -800) || (data_ < -3200))  
        result |= 0x04;
#endif
AKM_restore:
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data_[0] = 0x01;
    data_[1] = 0;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, data_))
        return -1;
    delay_ms(200);
    data_[0] = 0;
    if (i2c_write(st.hw->addr, st.reg->int_enable, 1, data_))
    ...
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, data_))
        return -1;
    data_[0] = BIT_FIFO_RST | BIT_DMP_RST;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, data_))
        return -1;
    delay_ms(15);
    data_[0] = st.test->reg_lpf;
    if (i2c_write(st.hw->addr, st.reg->lpf, 1, data_))
        return -1;
    data_[0] = st.test->reg_rate_div;
    if (i2c_write(st.hw->addr, st.reg->rate_div, 1, data_))
        return -1;
    if (hw_test)
        data_[0] = st.test->reg_gyro_fsr | 0xE0;
    else
        data_[0] = st.test->reg_gyro_fsr;
    if (i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, data_))
        return -1;

    if (hw_test)
        data_[0] = st.test->reg_accel_fsr | 0xE0;
    else
        data_[0] = test.reg_accel_fsr;
    if (i2c_write(st.hw->addr, st.reg->accel_cfg, 1, data_))
        return -1;
    if (hw_test)
        delay_ms(200);
    ...
    data_[0] = INV_XYZ_GYRO | INV_XYZ_ACCEL;
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, data_))
        return -1;
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (i2c_read(st.hw->addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH, data_))
            return -1;
        accel_cur[0] = ((short)data_[0] << 8) | data_[1];
        accel_cur[1] = ((short)data_[2] << 8) | data_[3];
        accel_cur[2] = ((short)data_[4] << 8) | data_[5];
        accel[0] += (long)accel_cur[0];
        accel[1] += (long)accel_cur[1];
        accel[2] += (long)accel_cur[2];
        gyro_cur[0] = (((short)data_[6] << 8) | data_[7]);
        gyro_cur[1] = (((short)data_[8] << 8) | data_[9]);
        gyro_cur[2] = (((short)data_[10] << 8) | data_[11]);
...
{
    unsigned char data_[HWST_MAX_PACKET_LENGTH];
    unsigned char packet_count, ii;
    unsigned short fifo_count;
    ...
    while (s < test.packet_thresh) {
		...
		if (i2c_read(st.hw->addr, st.reg->fifo_r_w, read_size, data_))
						return -1;
		ind = 0;
		for (ii = 0; ii < packet_count; ii++) {
			short accel_cur[3], gyro_cur[3];
			accel_cur[0] = ((short)data_[ind + 0] << 8) | data_[ind + 1];
			accel_cur[1] = ((short)data_[ind + 2] << 8) | data_[ind + 3];
			accel_cur[2] = ((short)data_[ind + 4] << 8) | data_[ind + 5];
			accel[0] += (long)accel_cur[0];
			accel[1] += (long)accel_cur[1];
			accel[2] += (long)accel_cur[2];
			gyro_cur[0] = (((short)data_[ind + 6] << 8) | data_[ind + 7]);
			gyro_cur[1] = (((short)data_[ind + 8] << 8) | data_[ind + 9]);
			gyro_cur[2] = (((short)data_[ind + 10] << 8) | data_[ind + 11]);
			gyro[0] += (long)gyro_cur[0];
			gyro[1] += (long)gyro_cur[1];
			gyro[2] += (long)gyro_cur[2];
    ...
    gyro[0] = (long)(gyro[0] / test.gyro_sens / s);
    gyro[1] = (long)(gyro[1] / test.gyro_sens / s);
    gyro[2] = (long)(gyro[2] / test.gyro_sens / s);
    accel[0] = (long)(accel[0] / test.accel_sens / s);
    accel[1] = (long)(accel[1] / test.accel_sens / s);
    accel[2] = (long)(accel[2] / test.accel_sens / s);
...
int mpu_get_compass_reg(short *data_, unsigned long *timestamp)
{
    ...
    if ((tmp[7] & AKM_OVERFLOW) || (tmp[7] & AKM_DATA_ERROR))
        return -3;
#elif defined AK8963_SECONDARY
    /* AK8963 doesn't have the data_ read error bit. */
    if (!(tmp[0] & AKM_DATA_READY) || (tmp[0] & AKM_DATA_OVERRUN))
        return -2;
    if (tmp[7] & AKM_OVERFLOW)
        return -3;
#endif
    data_[0] = (tmp[2] << 8) | tmp[1];
    data_[1] = (tmp[4] << 8) | tmp[3];
    data_[2] = (tmp[6] << 8) | tmp[5];

    data_[0] = ((long)data_[0] * st.chip_cfg.mag_sens_adj[0]) >> 8;
